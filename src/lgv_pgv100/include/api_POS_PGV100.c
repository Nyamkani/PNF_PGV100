...
uint16_t POS_GET_INFO(PosSensorVal* (PCVRSTValue)) 
{
     ...
     {
          PCVRSTValue->XPS = POS_X_COORDINATE;    // X 좌표
          PCVRSTValue->YPS = POS_Y_COORDINATE;    // Y 좌표 
...
uint16_t POS_CHKSUM_DATA()
{
     ...
     {//buf 0~20 8개(입력받는 21개 데이터-1) 
          if((even_cnt[i]&0x01)==0x01) ChkSum_Data|= ((u16)1<<i);//홀수이면 1, 짝수이면 0
     }
     return ChkSum_Data;
}
